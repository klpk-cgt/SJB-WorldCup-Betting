...
export function loadAdminSessions() {
  try {
    ...
    if (persistedSessions.length > 0) {
      ...
      logger.info(`[Admin] Loaded ${adminSessions.size} valid sessions from ${dbService.getStorageInfo().mode}.`);
    }
  } catch (error) {
...
export function appendSyncLog(log: SyncLog) {
  ...
  if (!log.requestSummary) log.requestSummary = log.action || 'Unknown';
  if (!log.createdAt) log.createdAt = new Date().toISOString();
  if (log.responseSummary === undefined || log.responseSummary === null || log.responseSummary === '') log.responseSummary = '-';
...
// ─── Quiz Data ───
export { quizQuestionPool, QUIZ_POINTS_PER_CORRECT };
